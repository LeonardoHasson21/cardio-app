...
export default function App() {
  ...
  // NUEVA CONSULTA (Valores por defecto correctos)
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <Card className="w-full max-w-md shadow-xl border-t-4 border-t-primary bg-white">
          ...
          <CardContent className="space-y-5 px-8 pb-8">
            <div className="space-y-2"><label className="text-sm font-medium text-gray-700">Email</label><Input className="h-11" placeholder="usuario@ejemplo.com" onChange={(e) => setLoginData({...loginData, username: e.target.value})} /></div>
            <div className="space-y-2"><label className="text-sm font-medium text-gray-700">Contraseña</label><Input className="h-11" type="password" placeholder="••••••" onChange={(e) => setLoginData({...loginData, password: e.target.value})} /></div>
  ...
  return (
    <div className="flex min-h-screen bg-muted/30">
        ...
        <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b px-6 h-16 flex items-center justify-between print:hidden">
          ...
          {activeTab === 'pacientes' && (
             <div className="hidden md:flex relative w-64">
                ...
                <Input placeholder="Buscar paciente..." className="pl-9 h-9 rounded-full" onChange={(e) => handleBusqueda(e.target.value)} />
             </div>
          )}
        </header>
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left">
                          <thead className="bg-gray-50/50 text-gray-500 font-medium border-b border-gray-100">
                            <tr><th className="px-6 py-4">ID</th><th className="px-6 py-4">PACIENTE</th><th className="px-6 py-4">FECHA</th><th className="px-6 py-4">TIPO</th><th className="px-6 py-4">ESTADO</th></tr>
                          </thead>
                          <tbody className="divide-y divide-gray-100 bg-white">
                            {dashboardData.ultimasConsultas?.map((c) => (
                              <tr key={c.id} className="hover:bg-blue-50/50 transition-colors cursor-pointer group" onClick={() => setConsultaParaImprimir(c)}>
                                ...
                                <td className="px-6 py-4"><span className={cn("px-2.5 py-1 rounded-full text-xs font-medium border", c.estado === 'En proceso' ? "bg-blue-50 text-blue-700 border-blue-100" : c.estado === 'Pendiente' ? "bg-amber-50 text-amber-700 border-amber-100" : "bg-emerald-50 text-emerald-700 border-emerald-100")}>{c.estado || "Completada"}</span></td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                ...
                {pacientes.map((p) => (
                  <Card key={p.id} className="border-none shadow-sm hover:shadow-lg transition-all duration-200 group bg-white">
                    <CardHeader className="flex flex-row items-center gap-4 pb-4">
                       <div className="w-12 h-12 rounded-full bg-primary/10 text-primary flex items-center justify-center text-lg font-bold">{p.nombre.charAt(0)}{p.apellido.charAt(0)}</div>
                       <div><CardTitle className="text-base font-bold text-gray-800">{p.nombre} {p.apellido}</CardTitle><p className="text-xs font-mono text-muted-foreground bg-gray-100 px-2 py-0.5 rounded w-fit mt-1">ID: {p.dni}</p></div>
                    </CardHeader>
                    <CardContent>
      ...
      {/* ================= MODAL IMPRESIÓN PDF (CON SCROLL + FIXED) ================= */}
      {consultaParaImprimir && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 print:p-0 print:bg-white print:static">
          {/* Wrapper con max-height para que scrollee en pantalla pequeña */}
          <div className="bg-white w-full max-w-3xl rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] print:max-h-none print:shadow-none print:rounded-none print:w-full print:h-full print:overflow-visible">
            ...
            <div className="p-10 overflow-y-auto print:p-0 print:overflow-visible text-gray-800 font-sans" id="documento-impresion">
              ...
              <div className="flex justify-between items-start border-b-2 border-gray-800 pb-6 mb-8">
                <div className="flex items-center gap-4">
                   <div className="w-16 h-16 bg-primary text-white rounded-lg flex items-center justify-center print:border print:border-gray-300 print:text-black">
                      <Activity className="w-10 h-10" />
                   </div>
                   <div><h1 className="text-2xl font-bold tracking-tight">MediRecord</h1><p className="text-sm text-gray-500">Informe Médico Oficial</p></div>
                </div>
              ...
              <div className="space-y-8">
                 <div className="grid grid-cols-2 gap-4 mb-4">
                    <div><h4 className="text-xs font-bold text-gray-400 uppercase">Tipo</h4><p className="font-medium">{consultaParaImprimir.tipo}</p></div>
                    <div><h4 className="text-xs font-bold text-gray-400 uppercase">Estado</h4><p className="font-medium">{consultaParaImprimir.estado}</p></div>
                 </div>
      ...
      {/* --- MODAL NUEVA EVOLUCIÓN (DROPDOWNS AGREGADOS) --- */}
            ...
            <div className="flex-1 overflow-auto p-6 space-y-8 bg-white">
              ...
              <div className="bg-blue-50/50 p-5 rounded-xl border border-blue-100 shadow-sm">
                ...
                <div className="space-y-3">
                  ...
                  {/* SELECTORES DE TIPO Y ESTADO (REQUERIDO) */}
                  <div className="grid grid-cols-2 gap-4">
                     <div className="space-y-1">
                        <label className="text-xs font-semibold text-blue-800 ml-1">Tipo de Consulta</label>
                        <div className="relative">
                            <select 
                                className="w-full h-10 rounded-md border border-input bg-white px-3 py-2 text-sm shadow-sm focus:ring-1 focus:ring-ring appearance-none"
                     ...
                     <div className="space-y-1">
                        ...
                        <div className="relative">
                            <select 
                                ...
                                onChange={(e)=>setNuevaConsulta({...nuevaConsulta, estado: e.target.value})}
                            >
                  ...
                  <div key={c.id} className="relative pl-6 border-l-2 border-gray-200 pb-6 last:pb-0">
                    ...
                    <div className="p-4 rounded-xl border bg-gray-50/50">
                      <div className="flex justify-between items-start mb-2">
                          <div>
                              <span className="font-bold text-gray-800 block">{c.motivo}</span>
                              <span className="text-xs text-blue-600 font-medium">{c.tipo} • {c.estado}</span>
                          </div>
